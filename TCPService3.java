...
public class TCPService3 {
    ...
    private void buildConnect() {
        try {
            lfd = new Socket(SERVICE_IP, TCPLFD3.SERVICE_PORT);
            OutputStream out = lfd.getOutputStream();
            String connectiongMsg = "3 connect" + END_CHAR;
    ...
    public class LFDTask implements Runnable{
        ...
        public void run() {
            while(true) {
                try {
                    StringBuilder receiveMsg = new StringBuilder();
                    InputStream in = TCPService3.lfd.getInputStream();
                    for (int c = in.read(); c != END_CHAR; c = in.read()) {
                        if(c ==-1)
                            break;
                        receiveMsg.append((char)c);
                    ...
                    OutputStream out = TCPService3.lfd.getOutputStream();
                    out.write(response.getBytes());
                }catch (Exception e){
                    e.printStackTrace();
                }
